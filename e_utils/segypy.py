"""
A python module for reading/writing/plotting/manipulating
...
-------------------- FIXES ------------------------------
-> 387 SH["ntraces"] = np.int(ntraces)       ------------------------>        SH["ntraces"] = int(ntraces) 
"""
# segypy : A Python module for reading and writing SEG-Y formatted data
#
# (C) Thomas Mejer Hansen, 2005-2018
# (C) Lots of stuff added by Musab al Hasani and Nicolas Vinard 2018
#
# contributions from
# - Pete Forman
#
# modified by N. Vinard and M. al Hasani 2019
...
##################################################
######### SEGY READING: HEADERS AND DATA #########
##################################################
def readSegy(filename):  # modified by A Squelch

    """

    Description:
       Function to read segy data. Only little endian supported

    Use:
       Data, SegyHeader, SegyTraceHeaders = readsegy(filename)
    ...
    SegyHeader = getSegyHeader(filename, endian)  # modified by A Squelch
    ...
    SegyHeader["ntraces"] = int(ntraces);

    printverbose("readSegy : ntraces=" + str(ntraces) + " nsamples=" + str(SegyHeader['ns']), 2)
    ...
    Data, SegyHeader, SegyTraceHeaders = readSegyData(data, SegyHeader, nd, bps, index, endian)

    printverbose("readSegy :  Read segy data", 2)  # modified by A Squelch

    SegyTraceHeaders = getSortingHeaders(SegyTraceHeaders)

    return Data, SegyHeader, SegyTraceHeaders
...
##################################################################
######### SEGY READING RELATED FUNCTIONS USED INTERNALLY #########
##################################################################
...
def getSortingHeaders(STH):

    traceNum = STH['TraceNumber']
    offset = STH['offset']
    SourceX = STH['SourceX']/1000 # scale to meters
    GroupX = STH['GroupX']/1000 # scale to meters
    cdpX = STH['cdp']

    return np.array([traceNum, SourceX, GroupX, cdpX, offset])
# %%
##############
# MISC FUNCTIONS
